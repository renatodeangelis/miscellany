...
m_min <- 1/2 # Slope of y_min
y_max <- m_max * x - 2
y_min <- m_min * x + 2

# Generate upward-sloping time series
generate_bounded_series <- function(n, y_min, y_max, slope, offset, noise_sd = 2) {
  y <- numeric(n)
...
# Adjusted slopes and offsets for distinct series
y_it <- generate_bounded_series(n, y_min, y_max, slope = 1.4, offset = 1.5)
y_jt <- generate_bounded_series(n, y_min, y_max, slope = 0.8, offset = 0.5)
...
ggplot() +
  ...
  labs(
    x = expression(W[t]),
    y = expression(W[t+1]),
    title = "Unstable Wealth Inequality in a Growing Economy"
  ) +
